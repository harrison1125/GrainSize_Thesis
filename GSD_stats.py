"""
GSD_statistics.py  —  SANDBOX VERSION of azimuthal_ring_statistics.py
-----------------------------------------------------------------------
Identical processing pipeline to azimuthal_ring_statistics.py with one
addition: a sixth row (Row 5) is appended to each per-image diagnostic PNG
showing, for each ring column, a histogram of the intensity values at the
individual peaks detected by find_peaks (the n_texture_peaks spots).

Each bar in the histogram corresponds to one detected peak; the x-axis is
the intensity of that peak in the azimuthal mean profile (mean_i[peak_idx]).
For scan points with zero detected peaks the panel is left blank with a note.

Because this extends plot_ring_metrics, a local replacement
(_plot_ring_metrics_with_histogram) is defined here rather than modifying
the shared ring_metrics module.

Run independently (does not modify any existing output):
    python GSD_statistics.py
"""
...
def _plot_ring_metrics_with_histogram(
    rings: list[dict],
    base_name: str,
    output_png: str,
) -> None:
    """
    Plot ring diagnostics with an extra Row 5: histogram of per-peak intensities.
    ...
    Row 5 (new):
      For each ring column, a histogram of mean_i evaluated at the index of
      every peak detected by find_peaks (the n_texture_peaks detections).
      x-axis: intensity at each detected peak position.
      y-axis: count (number of peaks).
      Panel is left blank with a note when n_texture_peaks == 0.
    ...
    fig, axes = plt.subplots(
        ...
        gridspec_kw={"height_ratios": [3, 1, 3, 2, 2, 2]},
        squeeze=False,
    )
...
def GSD_statistics(
    ...
    tree, producing the same outputs as azimuthal_ring_statistics() plus a
    per-peak intensity histogram row in each per-image diagnostic PNG.
    ...
    for dirpath, file_list in subfolders.items():
        ...
        for scan_point, filename, base_name in file_list:
            ...
            try:
                ...
                # Use local plot function (rows 0-4 unchanged, row 5 new)
                _plot_ring_metrics_with_histogram(rings_data, base_name, output_png)
